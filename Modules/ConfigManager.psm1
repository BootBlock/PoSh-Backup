    ...
    if ($finalConfiguration.ContainsKey('BackupTargets')) {
        if ($finalConfiguration.BackupTargets -isnot [hashtable]) {
            ...
            foreach ($targetName in $finalConfiguration.BackupTargets.Keys) {
                ...
                # MODIFIED: Only check for presence of TargetSpecificSettings. Type validation is for PoShBackupValidator.
                if (-not $targetInstance.ContainsKey('TargetSpecificSettings')) { 
                    $validationMessages.Add("ConfigManager: BackupTarget instance '$targetName' is missing 'TargetSpecificSettings'.")
                }
                # Further type/content validation of TargetSpecificSettings is done by PoShBackupValidator.psm1 or by the target provider itself.
            }
        }
    } # No 'else' needed here; BackupTargets is optional.

    # Advanced Schema Validation (if enabled and basic checks passed so far)
...
function Get-JobsToProcess {
    ...
    elseif (-not [string]::IsNullOrWhiteSpace($SpecifiedJobName)) {
        ...
        elseif ($jobCount -eq 0) {
            ...
            $errorMessageText = "ConfigManager: No job/set specified. Multiple Backup Locations defined. Please choose one:" # Renamed variable
            ...
            return @{ Success = $false; ErrorMessage = "$($errorMessageText)$($availableJobsMessage)$($availableSetsMessage)" } # Use renamed variable
        }
    }
