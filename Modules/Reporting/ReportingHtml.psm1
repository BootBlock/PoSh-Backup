...
.NOTES
    ...
    Version:        1.2.4 (Removed PSSA attribute suppression; trailing whitespace removed)
...
function Invoke-HtmlReport {
    ...
    if (-not (Test-Path -Path $ReportDirectory -PathType Container)) {
        & $LocalWriteLog -Message "[ERROR] HTML Report directory '$ReportDirectory' does not exist. Report cannot be generated for job '$JobName'." -Level "ERROR"
        return
    }
    ...
    if ([string]::IsNullOrWhiteSpace($mainScriptRoot) -or -not (Test-Path $mainScriptRoot -PathType Container)) {
        & $LocalWriteLog -Message "[ERROR] Main script root path ('_PoShBackup_PSScriptRoot') not found or invalid in GlobalConfig. Cannot load theme CSS files from 'Config\Themes'. Report styling will be minimal." -Level "ERROR"
        ...
        if (Test-Path -LiteralPath $baseCssFilePath -PathType Leaf) {
            ...
            catch { & $LocalWriteLog -Message "[WARNING] Could not load Base.css from '$baseCssFilePath': $($_.Exception.Message)" -Level "WARNING" }
        } else { & $LocalWriteLog -Message "[WARNING] Base.css not found at '$baseCssFilePath'. Report styling will heavily rely on theme or be minimal." -Level "WARNING" }
        ...
        if (-not [string]::IsNullOrWhiteSpace($reportThemeName)) {
            ...
            if (Test-Path -LiteralPath $themeCssFilePath -PathType Leaf) {
                ...
                catch { & $LocalWriteLog -Message "[WARNING] Could not load theme CSS '$safeThemeFileNameForFile' from '$themeCssFilePath': $($_.Exception.Message)" -Level "WARNING" }
            } else {
                & $LocalWriteLog -Message "[WARNING] Theme CSS file '$safeThemeFileNameForFile' for theme '$reportThemeName' not found at '$themeCssFilePath'." -Level "WARNING"
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($reportCustomCssPathUser) -and (Test-Path -LiteralPath $reportCustomCssPathUser -PathType Leaf)) {
            ...
            } catch { & $LocalWriteLog -Message "[WARNING] Could not load user custom CSS from '$reportCustomCssPathUser'. Error: $($_.Exception.Message)" -Level "WARNING" }
        }
...
"@
    ...
    if (-not [string]::IsNullOrWhiteSpace($reportLogoPath) -and (Test-Path -LiteralPath $reportLogoPath -PathType Leaf)) {
        ...
        } catch { & $LocalWriteLog -Message "[WARNING] Could not embed logo from '$reportLogoPath': $($_.Exception.Message)" -Level "WARNING"}
    }
    ...
    try {
        ...
        & $LocalWriteLog -Message "[ERROR] Failed to generate HTML report '$reportFullPath'. Error: $($_.Exception.Message)" -Level "ERROR"
    }
}
#endregion
