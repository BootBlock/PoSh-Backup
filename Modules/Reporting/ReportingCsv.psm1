...
.SYNOPSIS
    ...
    It provides a summary of the backup job (now including checksum details and
    multi-volume split size if applicable), detailed log entries, hook script
...
.DESCRIPTION
    ...
    1. A main summary CSV file (e.g., 'JobName_Summary_Timestamp.csv') containing
       key-value pairs of the overall job statistics and outcomes, including archive
       checksum information and split volume size if generated/configured.
    ...
    4. Optionally, if remote target transfers were attempted, a separate CSV file
...
.NOTES
    ...
    Version:        1.2.3 # SplitVolumeSize now implicitly included in ReportData for Summary CSV.
...
function Invoke-CsvReport {
    ...
    .DESCRIPTION
        ...
        now including checksum details and split volume size if applicable. If log entries,
        hook script data, or target transfer data exist, they are exported into their own
    ...
    .PARAMETER ReportData
        ...
        This function will extract summary information (including checksum and split volume data),
        log entries, hook script details, and target transfer details from this hashtable
        to populate the CSV files.
    .PARAMETER Logger
    ...
    try {
        ...
        # Checksum and SplitVolumeSize fields will be automatically included
        # as they are top-level keys in $ReportData.TargetTransfers
