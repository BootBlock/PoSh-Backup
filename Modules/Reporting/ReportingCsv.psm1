...
.SYNOPSIS
    ...
    It provides a summary of the backup job (now including checksum details),
    detailed log entries, hook script execution details, and remote target
    transfer details, each in separate, easily parsable CSV files.
...
.NOTES
    ...
    Version:        1.2.2 # Added Checksum information to Summary CSV.
...
function Invoke-CsvReport {
    ...
    .DESCRIPTION
        ...
        now including checksum details if available. If log entries, hook script data,
        or target transfer data exist, they are exported into their own respective CSV files.
        Files are named using the job name and a timestamp.
    ...
    .PARAMETER ReportData
        ...
        This function will extract summary information (including checksum data), log entries,
        hook script details, and target transfer details from this hashtable to populate the CSV files.
    .PARAMETER Logger
    ...
    try {
        ...
        # The new checksum fields (ArchiveChecksum, etc.) will be automatically included
        # as they are top-level keys in $ReportData.
