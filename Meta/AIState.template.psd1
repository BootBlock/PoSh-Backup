...
@{
  ...
  ai_development_watch_list = @(
    "CRITICAL (AI): Ensure full, untruncated files are provided when requested by the user. AI has made this mistake multiple times, including during the most recent session regarding CSS and Validator module updates, and most recently with RetentionManager.psm1.", # EMPHASIS ADDED
    "CRITICAL (AI): Verify line counts and comment integrity when AI provides full script updates; inadvertent removal/truncation has occurred (e.g., missing comments, fewer lines than expected). This was a significant issue in the last session, particularly with RetentionManager.psm1.", # EMPHASIS ADDED
    ...
    "DATA FLOW: Ensure data for reports (like `IsSimulationReport`, `OverallStatus`, `VSSStatus`, `VSSAttempted`) is correctly set in `theReportDataRefVariable` (a ref object) *before* report generation functions are called.", # Updated for VSSStatus/VSSAttempted
    ...
    "LOGIC (CONFIRMATION): The interaction between `[CmdletBinding(SupportsShouldProcess=$true, ConfirmImpact='...')`, `$PSCmdlet.ShouldProcess()`, `$ConfirmPreference`, and explicit `-Confirm` parameters on both the function call and internal cmdlets (like `Remove-Item`) is complex. Carefully test confirmation flows, especially when aiming for conditional suppression of prompts based on configuration." # NEW ITEM
  )
