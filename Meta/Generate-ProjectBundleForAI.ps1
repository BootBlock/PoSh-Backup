...
.EXAMPLE
    ...
    Generates 'ProjectBundle-YYYYMMDD_HHMMSS.txt' in the Meta folder.
    Includes PSScriptAnalyzer results AND PoSh-Backup.ps1 -TestConfig output by default.

.EXAMPLE
    .\Generate-ProjectBundleForAI.ps1 -NoRunScriptAnalyzer -DoNotIncludeTestConfigOutput
    Generates the bundle, skips PSScriptAnalyzer, AND skips PoSh-Backup.ps1 -TestConfig output.

.NOTES
    Author: Your Name/AI Assistant
    Version: 1.15.3 (Simplified -TestConfig output capture with Invoke-Expression and stream redirection)
    Date: 15-May-2025
#>
...
try {
    # ... (file bundling logic - same as before) ...
    ...
    # ... (AI State generation - same as before) ...
    ...
    $bundlerScriptVersion = "N/A" # Default
    if ($thisBundlerScriptFileObject) {
        $bundlerScriptVersion = Get-ScriptVersionFromContent -ScriptContent (Get-Content -LiteralPath $thisBundlerScriptFileObject.FullName -Raw -ErrorAction SilentlyContinue) -ScriptNameForWarning $thisBundlerScriptFileObject.Name
    } else {
         Write-Warning "Bundler: Could not get bundler script file object for version extraction."
    }
    ...
    $aiState = @{
        ...
        # ... (rest of aiState hashtable definition) ...
        ...
        ai_development_watch_list = @( 
            "CRITICAL: Ensure full, untruncated files are provided when requested by the user. AI has made this mistake multiple times.",
            "SYNTAX: PowerShell ordered dictionaries (`[ordered]@{}`) use `(\$dict.PSObject.Properties.Name -contains 'Key')`, NOT `\$dict.ContainsKey('Key')`.",
            "SYNTAX: For literal backticks (```) in PowerShell strings meant for Markdown, use single quotes: `'```'`.",
    ...
    if ($shouldIncludeTestConfigOutput) { 
        ...
        if (Test-Path -LiteralPath $fullPathToPoShBackupScript -PathType Leaf) {
            try {
                ...
                $testConfigOutput = ""
                $LASTEXITCODE = 0 # Reset for this operation

                Push-Location (Split-Path -Path $fullPathToPoShBackupScript -Parent)
                try {
                    # Using Invoke-Expression to run the script in the current PowerShell process,
                    # redirecting all output streams (*>) to the success stream (1), then capturing.
                    $invokeCommand = ". `"$fullPathToPoShBackupScript`" -TestConfig *>&1"
                    $testConfigOutput = Invoke-Expression $invokeCommand | Out-String
                }
                catch {
                    # This catch is for script-terminating errors from Invoke-Expression itself or PoSh-Backup.ps1
                    $testConfigOutput = "INVOKE-EXPRESSION FAILED: $($_.Exception.ToString())`n$($_.ScriptStackTrace)"
                    # Try to get $Error[0] if available for more detailed script error
                    if ($Error.Count -gt 0) {
            ...
            } catch { # Catch errors from the bundler's main try block around this operation
    ...
    if ($shouldRunScriptAnalyzer) { 
        # ... (PSScriptAnalyzer logic - same as before) ...
            ...
            try {
                ...
                if ($scriptFilesToAnalyze.Count -gt 0) {
                    ...
                    foreach ($scriptFile in $scriptFilesToAnalyze) {
                        ...
                        $analyzerResultsForFile = Invoke-ScriptAnalyzer -Path $scriptFile.FullName -Severity Error, Warning -ErrorAction SilentlyContinue
                    ...
                    if ($allAnalyzerResultsList.Count -gt 0) {
                        ...
                        $null = $finalOutputBuilder.AppendLine("(No PSScriptAnalyzer errors or warnings found in .ps1/.psm1 files.)")
                    }
                } else {
    ...
    try {
        ...
        Write-Host "Project bundle successfully written to: $outputFilePath" -ForegroundColor Green
    } catch {
